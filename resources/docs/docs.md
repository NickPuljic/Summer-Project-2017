...
The `home-routes` handler in the `nuenen.routes.home` namespace
...
The `home-page` function will in turn call the `nuenen.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/nuenen/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `nuenen.handler` namespace:
...
Request middleware functions are located under the `nuenen.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `nuenen.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `nuenen.db.core` in some other namespace.
